...
def train(env, device, num_episodes, buffer_depth, batch_size,
    ...
    steps_done = 0
    # episode_lengths = []
    rewards_eval_episode = np.zeros(num_episodes)
    ...
    # plt.plot(rewards_eval_episode , label = "temp = " + str(temp))
    # plt.xlabel('Episode')
    # plt.ylabel('Rewards'
    # plt.title(' yea Rewards per episode')
    # plt.show()
...
def main():
    ...
    for temp in temps:
        ...
        for n in range(0, n_repetitions):
            print(f"Currently at repetition {n}")
        ...
        print(reward_curve_single_setting)
        reward_curve_mean = np.mean(reward_curve_single_setting, axis=0)
        reward_curve_smooth = savgol_filter(reward_curve_mean, 11, polyorder=2)
        plt.plot(reward_curve_smooth, label="T = " + str(temp))
        print("Completed the evaluation with T = ", str(temp))
