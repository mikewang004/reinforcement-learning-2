...
def experiment_discount_factor(epsilon_start, epsilon_end, epsilon_decay, env, device, num_episodes, network_sizes, gamma, n_repetitions = 20):
    repetition_array = np.zeros(n_repetitions)
    #reward_curve_repetitions = np.zeros([n_repetitions, reward_eval_count, len(epsilon_decay) * len(epsilon_end) * len(epsilon_start)]) #Store reward curves here
    reward_curve_repetitions = np.zeros([num_episodes, len(gamma)])
    i = 0; epss = epsilon_start; epse = epsilon_end; epsd = epsilon_decay
    for g in gamma:
        for n in range(0, n_repetitions):
            print(f"Current at repetition {n}")
        ...
        reward_curve_repetitions[:, i] = np.mean(reward_curve_single_setting, axis = 0)
        i = i + 1
        print(f"Current at iteration {i} of {len(gamma)}, with learning rate {g}")
    #Take average of n_repetitions
    reward_curve = reward_curve_repetitions #Note episodes at axis 0; num iterations at axis 1
    #Plot reward curves 
    i = 0
    np.savetxt(f"data/gamma_experiment_{num_episodes}_n_reps_{n_repetitions}.txt", reward_curve)
    plt.figure()
    plt.title("Comparison of various discount factors")
    plt.xlabel("episode"); plt.ylabel("rewards")
    #plt.xticks(np.linspace(0, num_episodes, reward_eval_count+1))
    for g in gamma:
            plt.plot(np.linspace(0, num_episodes, num_episodes+1)[:-1], reward_curve[:, i], label = f"learning rate = {g}")
            i = i + 1
    plt.legend(loc='upper left', prop={'size': 6})
    plt.savefig(f"plots/gamma_experiment_{num_episodes}_n_reps_{n_repetitions}.pdf")
...
def main():
    ...
    experiment_discount_factor(epsilon_start, epsilon_end, epsilon_decay, env, device, num_episodes, network_sizes, gamma, n_repetitions)
